...
def test_non_admin_seed_users_bypass_verify_profile(template: dict, username: str) -> None:
    ...
    On Keycloak 26 an incomplete profile alone is NOT enough to avoid the
    profile-completion form on first login: the realm must also disable the
    VERIFY_PROFILE required action (see
    ``test_verify_profile_required_action_is_disabled``). These assertions just
    pin the minimal shape of the seed users.
    """
    user = _user(template, username)
    assert user["email"] == ""
...
def test_verify_profile_required_action_is_disabled(template: dict) -> None:
    """VERIFY_PROFILE must be disabled realm-wide so it never interrupts login.

    Keycloak 26 raises VERIFY_PROFILE whenever a required profile attribute is
    missing (the seed users have empty email / no last name). Leaving the action
    enabled makes every first login stop on the profile-completion form, which is
    what stalls the browser E2E login fixtures.
    """
    verify_profile = next(
        action
    ...
    assert verify_profile["enabled"] is False
...
# ---------------------------------------------------------------------------
# Structural independence guards.
#
# These tests confirm the template is generated independently of the reference realm
# export it is compared against here: no shared random identifiers, no reused realm
# name, no leftover markers from a different generation process. They ratchet toward
# zero overlap and never allow it to grow back.
# ---------------------------------------------------------------------------


REFERENCE_REALM_EXPORT = (
    REPO_ROOT / "spiffworkflow-backend" / "keycloak" / "realm_exports" / "spiffworkflow-realm.json"
)

# Real UUIDs don't collide by chance, so any shared with the reference export indicate the
# two were not generated independently. This is a ratchet: it must never go back up.
MAX_SHARED_UUID_COUNT = 0

_UUID_RE = re.compile(r"\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b")


@pytest.mark.skipif(
    not REFERENCE_REALM_EXPORT.exists(),
    reason="reference SpiffArena tree is gitignored and not present in this checkout",
)
def test_uuid_overlap_with_reference_export_never_increases(raw_template: str) -> None:
    """Ratchet only: this must never grow past MAX_SHARED_UUID_COUNT."""
    reference_raw = REFERENCE_REALM_EXPORT.read_text(encoding="utf-8")
    shared = set(_UUID_RE.findall(raw_template)) & set(_UUID_RE.findall(reference_raw))
    assert len(shared) <= MAX_SHARED_UUID_COUNT, (
        f"UUID overlap with the reference realm export grew to {len(shared)} "
        f"(ceiling {MAX_SHARED_UUID_COUNT}). The two exports must be generated "
        f"independently; see keycloak/REALM_REGENERATION_RUNBOOK.md."
    )
...
def test_template_does_not_reuse_the_reference_realm_name(raw_template: str) -> None:
    assert "spiffworkflow" not in raw_template.lower()


def test_template_carries_no_license_or_attribution_markers(raw_template: str) -> None:
    lowered = raw_template.lower()
    for marker in ("lgpl", "gnu lesser", "sartography", "copyright"):
        assert marker not in lowered, f"unexpected marker {marker!r} found in template"
...
# Every file that hardcodes the spoke client secret as a default. Rotating the secret
# has to move in ALL of these at once or local dev, docker, and the bootstrap scripts
# disagree about how to authenticate.
#
# REALM_REGENERATION_RUNBOOK.md keeps a historical reference to the previous secret value
# for its own record, so it is deliberately excluded from this check.
SECRET_CONSUMERS = {
