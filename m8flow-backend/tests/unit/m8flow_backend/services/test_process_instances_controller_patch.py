...
def _install_controller_fakes(
    ...
    *,
...
) -> tuple[ModuleType, ModuleType, list, list]:
    ...
    fake_controller_module = ModuleType(
    ...
    fake_tmp_service_module = ModuleType(
        "spiffworkflow_backend.services.process_instance_tmp_service"
    )
    fake_error_handling_module = ModuleType(
    ...
    fake_queue_module.queue_process_instance_if_appropriate = (
        fake_queue_process_instance_if_appropriate
    )
    fake_queue_module.should_queue_process_instance = lambda execution_mode=None: (
        should_queue
    )
    ...
    fake_tmp_service_module.ProcessInstanceTmpService = SimpleNamespace(
        is_enqueued_to_run_in_the_future=lambda process_instance: already_enqueued,
    )
    ...
    def upstream_shaped_process_instance_run(
        ...
        try:
            ...
            fake_controller_module.ErrorHandlingService.handle_error(
                process_instance, error
            )
            raise
...
def test_apply_preflights_queued_process_start_before_queueing(monkeypatch) -> None:
    ...
    fake_controller_module, _fake_queue_module, queue_calls, handle_error_calls = (
        _install_controller_fakes(monkeypatch)
    )
    ...
    monkeypatch.setattr(
        ...
        lambda process_instance, handle_error=True: preflight_calls.append(
            (process_instance, handle_error)
        ),
    )
    ...
    fake_controller_module._process_instance_run(
        process_instance, execution_mode="asynchronous"
    )
...
def test_process_run_does_not_fault_instance_for_task_lane_assignment_api_error(
    monkeypatch,
) -> None:
    from m8flow_backend.services import process_instance_service_patch
    from m8flow_backend.services import process_instances_controller_patch

    fake_controller_module, _fake_queue_module, _queue_calls, handle_error_calls = (
        _install_controller_fakes(monkeypatch)
    )
...
def test_apply_skips_preflight_when_process_should_not_queue(monkeypatch) -> None:
    from m8flow_backend.services import process_instance_service_patch
    from m8flow_backend.services import process_instances_controller_patch

    fake_controller_module, _fake_queue_module, queue_calls, handle_error_calls = (
        _install_controller_fakes(monkeypatch, should_queue=False)
    )
    preflight_calls: list[tuple[object, bool]] = []
    monkeypatch.setattr(
        process_instance_service_patch,
    ...
    process_instance = SimpleNamespace(id=8, status="not_started")
    fake_controller_module._process_instance_run(
        process_instance, execution_mode="synchronous"
    )

    assert preflight_calls == []
    assert queue_calls == [("queue", "synchronous")]
    assert handle_error_calls == []


def test_apply_skips_preflight_when_already_enqueued_for_the_future(
    monkeypatch,
) -> None:
    from m8flow_backend.services import process_instance_service_patch
    from m8flow_backend.services import process_instances_controller_patch
    ...
    fake_controller_module, _fake_queue_module, queue_calls, handle_error_calls = (
        _install_controller_fakes(monkeypatch, already_enqueued=True)
    )
    preflight_calls: list[tuple[object, bool]] = []
    monkeypatch.setattr(
        process_instance_service_patch,
...
def test_process_run_faults_instance_for_non_lane_assignment_api_error(
    monkeypatch,
) -> None:
    from m8flow_backend.services import process_instance_service_patch
    from m8flow_backend.services import process_instances_controller_patch
    ...
    def fake_preflight(process_instance, handle_error=False):  # noqa: ANN001
        raise FakeApiError("task_not_found", "missing task", 404)

    monkeypatch.setattr(
        process_instance_service_patch, "_validate_queued_process_start", fake_preflight
    )
    process_instances_controller_patch.apply()
    ...
    assert raised_error is not None
    assert raised_error.error_code == "task_not_found"
    assert handle_error_calls == [(process_instance, raised_error)]
