"""Model for tracking soft-deleted process groups."""
...
@dataclass
class ProcessGroupDeletionModel(M8fTenantScopedMixin, TenantScoped, SpiffworkflowBaseDBModel, AuditDateTimeMixin):
    """Tracks soft-deleted process groups for restore and purge lifecycle."""

    __tablename__ = "m8flow_process_group_deletion"
    __table_args__ = (
        Index("ix_pgd_m8f_tenant_id", "m8f_tenant_id"),
        Index("ix_pgd_original_identifier", "original_identifier"),
        Index("ix_pgd_status", "status"),
        Index("ix_pgd_deleted_at", "deleted_at_in_seconds"),
    )
