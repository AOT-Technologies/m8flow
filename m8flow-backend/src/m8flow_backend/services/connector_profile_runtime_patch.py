...
A service task opts in by carrying a ``m8flow_profile`` parameter (the modeler
writes it when an author picks a profile). At execution the profile's config
and secrets are resolved and merged into the call, so the BPMN never holds a
host name or a password.

Tasks without that parameter are passed through untouched, which is what keeps
every existing process model - including the shipped sample templates, which
spell out ``M8FLOW_SECRET:...`` per field - working exactly as before.
"""
...
# The connector proxy catalogue changes only when the proxy is redeployed.
...
def declared_parameter_names(operator_identifier: str) -> frozenset[str] | None:
    ...
    parameter it does not declare is a hard failure. Profiles are therefore
    injected only into the parameters the operator actually takes - n8n's
    TriggerWorkflow, for instance, takes neither base_url nor api_key.
    """
...
def _profile_name(entry: Any) -> str | None:
    ...
    engine, so the quoted expression in the XML arrives as a plain string.
    """
...
def apply() -> None:
    ...
    def patched_call_connector(cls, operator_identifier: str, bpmn_params: Any, spiff_task: Any) -> str:
        params = dict(bpmn_params or {})
        profile_name = _profile_name(params.pop(PROFILE_PARAMETER_NAME, None))

        if not profile_name:
            return original_call_connector(cls, operator_identifier, bpmn_params, spiff_task)

        connector_type = operator_identifier.split("/", 1)[0]
        resolved = ConnectorProfileService.resolve_for_runtime(connector_type, profile_name)
        ...
        for name, value in resolved.items():
            ...
            if not _is_unset(params.get(name)):
                # The author typed something for this parameter; their value wins.
                continue
