...
**Permissions and role alignment:** For the backend to grant API and UI permissions, Keycloak realm role names must match the group names defined in `m8flow.yml`: `super-admin`, `tenant-admin`, `editor`, `viewer`, `integrator`, `reviewer`. Tenant realm tokens now separate organizational and authorization membership: actual Keycloak group paths are emitted in the `groups` claim, and realm roles are emitted in a separate top-level `roles` claim for the **m8flow-backend** client. For master-realm admin tokens that do not include a top-level `roles` claim (for example `admin-cli` tokens), the backend still falls back to `realm_access.roles` and maps the matching M8Flow role names automatically.
