...
# Warning log registered tables after model overrides and before app creation
...
def _assert_db_engine_bound(app: Flask) -> None:
    """Ensure db.engine is bound inside app context."""
    from spiffworkflow_backend.models.db import db
    with app.app_context():
...
# Catches re-init/duplicate db/base during app creation
_assert_model_identity()
...
def _m8flow_startup_once():
    """Run m8flow startup tasks once, at first request."""
    global _started
    if _started:
        return
    _started = True
