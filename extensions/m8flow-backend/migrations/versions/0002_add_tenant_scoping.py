...
def _get_backfill_tenant_id() -> str:
    conn = op.get_bind()
    insp = _inspector()
    columns = [c["name"] for c in insp.get_columns("m8flow_tenant")]
    has_slug = "slug" in columns

    if has_slug:
        # Table was already evolved by a later migration (e.g. 2cfce680c743); include required columns.
        status_expr = "'ACTIVE'::tenantstatus" if _is_postgres() else "'ACTIVE'"
        conn.execute(
            sa.text(
                "INSERT INTO m8flow_tenant (id, name, slug, status, created_by, modified_by) "
                f"VALUES (:tenant_id, :tenant_name, :tenant_slug, {status_expr}, 'system', 'system') "
...
def upgrade() -> None:
    ...
    if _table_exists("bpmn_process_definition"):
    ...
    if _table_exists("task_definition"):
    ...
    if _table_exists("message_triggerable_process_model"):
    ...
    if _table_exists("json_data_store"):
    ...
    if _table_exists("kkv_data_store"):
    ...
    if _table_exists("process_instance_report"):
    ...
    if _table_exists("reference_cache"):
    ...
    if _table_exists("service_account"):
