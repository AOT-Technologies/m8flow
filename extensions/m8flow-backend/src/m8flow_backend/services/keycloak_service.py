...
def _apply_runtime_client_urls(
    ...
    *,
    ...
    is_spoke_backend_client = client_id == spoke_client_id()
    is_spoke_frontend_client = client_id == "spiffworkflow-frontend"

    redirect_uris = client.get("redirectUris")
    updated_redirect_uris: list[str] = []
    ...
    if is_spoke_backend_client:
        updated_redirect_uris.extend(candidate for candidate in (backend_wildcard, frontend_wildcard) if candidate)
    elif is_spoke_frontend_client:
        updated_redirect_uris.extend(candidate for candidate in (frontend_wildcard,) if candidate)
    ...
    if is_spoke_backend_client:
        updated_web_origins.extend(candidate for candidate in (backend_origin, frontend_origin) if candidate)
    elif is_spoke_frontend_client:
        updated_web_origins.extend(candidate for candidate in (frontend_origin,) if candidate)
    ...
    if isinstance(attrs, dict):
        ...
        if is_spoke_backend_client:
            post_logout_uris = _split_keycloak_uri_list(attrs.get("post.logout.redirect.uris"))
            post_logout_uris.extend(candidate for candidate in (frontend_wildcard, backend_wildcard) if candidate)
            attrs["post.logout.redirect.uris"] = "##".join(_unique_strings(post_logout_uris))
        elif is_spoke_frontend_client:
            post_logout_uris = _split_keycloak_uri_list(attrs.get("post.logout.redirect.uris"))
            post_logout_uris.extend(candidate for candidate in (frontend_wildcard,) if candidate)
            attrs["post.logout.redirect.uris"] = "##".join(_unique_strings(post_logout_uris))
