...
| **redis** | redis:6-alpine | Celery broker/result backend (optional). | `${REDIS_HOST_PORT:-6848}`→6379 | Persistence: `redis-data`. |
| **minio** | minio/minio (pinned) | S3-compatible object store for process models and templates. | `${MINIO_API_PORT:-6846}`→9000, `${MINIO_CONSOLE_PORT:-6847}`→9001 | `MINIO_ROOT_USER/PASSWORD` from `.env`. Data: volume `minio_data`. |
...
| **m8flow-frontend** | m8flow.frontend.Dockerfile | Nginx serving the built React app (core + extension). | `${M8FLOW_FRONTEND_PORT:-6841}`→8080 | Reads `.env` at build time (e.g. `MULTI_TENANT_ON`, `VITE_BACKEND_BASE_URL`). Listens on 8080 (non-root). Runs as user `nginx`. |
