...
| **nginx-keycloak-proxy.conf** | Nginx config for the keycloak-proxy service: listen **7002** (container), proxy to keycloak:8080. |
...
| **keycloak** | Built (m8flow.keycloak.Dockerfile) | IdP: auth, shared realm, admin realm, and realm-info-mapper support. | `${KEYCLOAK_MGMT_PORT:-7009}` -> 9000 (management) | `KEYCLOAK_ADMIN*`, `KC_DB_*`, `KC_HTTP_PORT` (8080), `KC_HOSTNAME` (user-facing URL). Dev: `start-dev --import-realm`; prod override uses `start --import-realm`. Realms imported from image. Runs as user `keycloak`. |
| **keycloak-proxy** | nginx:alpine | Reverse proxy so browser and backend use one URL for Keycloak. | `${KEYCLOAK_PROXY_PORT:-7002}` -> 7002 | Uses `nginx-keycloak-proxy.conf`: listen 7002, `proxy_pass` to keycloak:8080. |
...
|---------|-------|---------|-------|----------------|
| **m8flow-backend** | m8flow.backend.Dockerfile | Flask/uvicorn API (SpiffWorkflow + m8flow extensions). Runs migrations on startup. | `${M8FLOW_BACKEND_PORT:-7000}` -> `${M8FLOW_BACKEND_PORT:-7000}` | DB: `m8flow-db`. Keycloak: `keycloak-proxy:7002`. Redis/Celery, MinIO URLs set for Docker. BPMN/templates dirs: volumes `process_models_cache`, `templates_cache` at `/app/data/process_models`, `/app/data/templates`. Entrypoint chowns those dirs then runs app as user `app` (UID 1000). Default build target: `dev`; prod override uses target `prod`. |
| **m8flow-frontend** | m8flow.frontend.Dockerfile | Nginx serving the built React app (core + extension). | `${M8FLOW_FRONTEND_PORT:-7001}` -> 8080 | Reads `.env` at build time (for example `MULTI_TENANT_ON`, `VITE_BACKEND_BASE_URL`). Listens on 8080 (non-root). Runs as user `nginx`. |
...
Used by the **keycloak-proxy** service. Listens on port **7002** inside the container and proxies all traffic to **keycloak:8080**. Host mapping is `${KEYCLOAK_PROXY_PORT:-7002}:7002`. Users and the backend use `http://<host>:<KEYCLOAK_PROXY_PORT>` on the host; inside Compose the backend uses `KEYCLOAK_URL` / `M8FLOW_KEYCLOAK_URL` -> `http://keycloak-proxy:7002`.
...
- **Frontend:** Runs as user `nginx`, listens on **8080** inside the container; host port (default **7001**) is mapped to 8080.
