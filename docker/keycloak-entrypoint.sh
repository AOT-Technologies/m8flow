...
prepare_m8flow_realm_import() {
  ...
  sed -i \
    ...
    -e "s|https://replace-me-with-spiff-backend-host-and-path/\\*|${escaped_backend_redirect}|g" \
    -e "s|https://replace-me-with-spiff-frontend-host-and-path/\\*|${escaped_frontend_redirect}|g" \
