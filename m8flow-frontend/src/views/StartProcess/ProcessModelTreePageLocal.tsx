...
import ConfirmIconButton from '@spiff-core/components/ConfirmIconButton';
import DataStoreCard from '@spiff-core/components/DataStoreCard';
import TreePanel, { TreeRef, SHOW_FAVORITES } from '@spiff-core/views/StartProcess/TreePanel';
import SearchBar from '@spiff-core/views/StartProcess/SearchBar';
import { getStorageValue, SPIFF_FAVORITES } from '@spiff-core/services/LocalStorageService';
...
export default function ProcessModelTreePage({
  ...
  const workflowTenantSelectionRequired =
    UserService.isSuperAdmin() && !selectedTenantId;
  const workflowTenantSelectionMessage = t(
    'select_tenant_before_workflow_management',
  );
  const gridProps = {
    display: 'grid',
    gridGap: 20,
  ...
  function findProcessGroupByPath(
    ...
    let newGroups: ProcessGroupLite[] = groupsToProcess || [];
  ...
  useEffect(() => {
    ...
    setClickedItem(null);
  }, [clickedItem, flattenAllItems, navigate, processGroups]);
  ...
  }, [ability, permissionsLoaded, processGroups]);

  useEffect(() => {
    const streamSubscription = clickStream.subscribe(handleClickStream);
    return () => streamSubscription.unsubscribe();
  }, [clickStream]);

  const handleFavorites = ({ text }: { text: string }) => {
    if (text === SHOW_FAVORITES) {
      const storage = JSON.parse(getStorageValue(SPIFF_FAVORITES));
      const favorites = flatItems.filter((item: any) => storage.includes(item.id));
      setGroups(favorites.length ? [] : processGroups);
      setModels(favorites);
      setModelsExpanded(!!favorites.length);
      setGroupsExpanded(!favorites.length);
      setCrumbs([favoriteCrumb]);
    }
  ...
  const getParentGroupsDisplayName = (
    ...
    if (processItem.parent_groups) {
      return processItem.parent_groups
        .map((parentGroup: ProcessGroupLite) => parentGroup.display_name)
        .join(' / ');
    }
    return '';
  };
  ...
  const shouldFilterProcessModel = (
    ...
    return inputValueArray.every((item: any) =>
      processModelLowerCase.includes((item || '').toLowerCase()),
    );
  };
  ...
  const handleCrumbClick = (crumb: Crumb) => {
    ...
    if (found) {
      ...
      navigate(`/process-groups/${modifyProcessIdentifierForPathParam(crumb.id)}`);
    }
  };

  const dataStoresForProcessGroup = dataStores.filter((dataStore: DataStore) => {
    return (
      (currentProcessGroup &&
  ...
  const deleteProcessGroup = () => {
    ...
    let parentGroupId = modifiedGroupId.replace(/:[^:]+$/, '');
    if (parentGroupId === modifiedGroupId) {
      parentGroupId = SPIFF_ID;
    }
  ...
  const currentParentGroupIdSearchParam = () =>
    currentProcessGroup ? `?parentGroupId=${currentProcessGroup.id}` : '';
    ...
    <Box id="process-model-tree-box" sx={{ margin: '0 auto', p: 0 }}>
      ...
      >
        ...
        >
          ...
          {workflowTenantSelectionRequired ? (
            <Alert
              severity="warning"
              sx={{ width: '100%' }}
              data-testid="super-admin-workflow-tenant-alert"
            >
              {workflowTenantSelectionMessage}
            </Alert>
          ) : null}
          ...
          >
            ...
            <Card>
              <CardContent>
                ...
                {currentProcessGroup ? (
                  <Stack direction="row" sx={{ width: '100%', paddingBottom: 2 }}>
                            ...
                            >
                              <span>
                                <IconButton
                                  ...
                                  disabled={workflowTenantSelectionRequired}
                                >
                                  <Add />
                                </IconButton>
                              </span>
                  ...
                  <AccordionDetails>
                    <Box sx={gridProps}>
                      {models.map((model: ProcessModel) => (
                        <ProcessModelCard
                          key={model.id}
                          model={model as ProcessModel & { tenantName?: string }}
                          stream={clickStream}
                          lastSelected={currentProcessGroup || {}}
                          disableStartProcess={workflowTenantSelectionRequired}
                          disabledReason={workflowTenantSelectionMessage}
                          onStartProcess={() => {
