...
//
// Derived from spiffworkflow-frontend/src/views/ProcessInstanceShow.tsx in SpiffArena
...
/*
 * M8Flow override of spiffworkflow-frontend/src/views/ProcessInstanceShow.tsx.
 *
 * This is a verbatim copy of the upstream component with a SINGLE intentional
 * change: the layout produced by getInfoTag() (the summary details + instance
 * variables / process-metadata section) is restructured to align cleanly and
 * use the full available page width instead of being cramped into a fixed
 * half-width column. See M8F-370. All other logic is unchanged from upstream;
 * when re-syncing with upstream, diff getInfoTag() and re-apply only that block.
 */
...
import './ProcessInstanceShow.css';
...
export default function ProcessInstanceShow({ variant }: OwnProps) {
  ...
  const getInfoTag = () => {
    ...
    const processMetadata = processInstance.process_metadata || [];

    return (
      <Grid container spacing={2}>
        <Grid size={{ xs: 12 }}>
          <div className="m8f-info-grid">
            <dl>
              <Typography component="dt" variant="subtitle2">
                {t('status')}:
              </Typography>
        ...
        {processMetadata.length > 0 ? (
          <Grid size={{ xs: 12 }}>
            <div className="m8f-metadata-grid">
              {processMetadata.map((processInstanceMetadata) => (
                <dl
                  className="metadata-display"
                  key={processInstanceMetadata.key}
                >
                  <Typography
                    component="dt"
                    variant="subtitle2"
