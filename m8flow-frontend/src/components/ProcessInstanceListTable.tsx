...
type ProcessInstanceWithTenant = ProcessInstance & {
  tenantName?: string;
  tenantId?: string;
  tenant_name?: string;
};
...
export default function ProcessInstanceListTable({
  ...
  const tenantLabel = (processInstance: ProcessInstance) => {
    const row = processInstance as ProcessInstanceWithTenant;
    return row.tenantName || row.tenant_name || row.tenantId || '-';
  };

  const tenantCell = (processInstance: ProcessInstance) => {
    return (
      <TableCell
        key={`td-tenantName-${processInstance.id}`}
        onClick={() => navigateToProcessInstance(processInstance)}
        onKeyDown={() => navigateToProcessInstance(processInstance)}
        data-testid="process-instance-show-link-tenantName"
      >
        {tenantLabel(processInstance)}
      </TableCell>
    );
  };
  ...
  const buildTable = () => {
    ...
    const rows = processInstances.map((processInstance: ProcessInstance) => {
      ...
      if (isSuperAdmin) {
        currentRow.splice(1, 0, tenantCell(processInstance));
      }
      if (showActionsColumn) {
        ...
        if (!(processInstance.status in statusesToExcludeTaskButton)) {
          currentRow.push(
            <TableCell align="right" key={`td-actions-${processInstance.id}`}>
              {goButtonElement}
              {piShowButtonElement}
            </TableCell>,
          );
          ...
          currentRow.push(
            <TableCell key={`td-actions-${processInstance.id}`}>
              {piShowButtonElement}
            </TableCell>,
          );
        }
