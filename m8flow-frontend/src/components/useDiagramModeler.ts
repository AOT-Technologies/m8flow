...
export function useDiagramModeler(options: UseDiagramModelerOptions) {
  ...
  useEffect(() => {
    ...
    if (diagramType === 'bpmn') {
      diagramModeler = new BpmnModeler({
        ...
        additionalModules: [
          spiffworkflow,
          m8flowExternalForm,
          // After spiffworkflow: it rewrites the service task group that
          // module builds.
    ...
    return () => {
      ...
      // Connector profiles are tenant data cached in a module, which outlives
      // this modeler. Drop it so the next one reloads for its own tenant.
