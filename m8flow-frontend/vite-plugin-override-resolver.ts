 ...
 * 1. When core files import each other, check for overrides in m8flow-frontend/src
 * 2. When importing from @spiffworkflow-frontend, check for overrides first
 * 3. Bare module imports from core files resolve to m8flow-frontend/node_modules
 */
...
export function overrideResolver(): Plugin {
  ...
  return {
    ...
    resolveId(source, importer, options) {
      ...
      const importerInCore = importerNorm.includes('/spiffworkflow-frontend/src/');
      const importerInM8flowFrontend = importerNorm.includes('/m8flow-frontend/src/');
      ...
      // Handle relative imports from m8flow-frontend files
      if (importerInM8flowFrontend && source.startsWith('.')) {
        const importerDir = path.dirname(importer);
        const resolvedPath = path.resolve(importerDir, source);

        // Check if resolved path is within m8flow-frontend/src
        if (resolvedPath.startsWith(extensionsDir)) {
          // Get relative path from m8flow-frontend src
