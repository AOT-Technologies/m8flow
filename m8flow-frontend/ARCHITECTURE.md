...
The `m8flow-frontend` is a **standalone React application** that extends and overrides components from `spiffworkflow-frontend` without requiring any modifications to the core application. This architecture enables:
...
- **Hot Module Replacement**: Changes in both m8flow-frontend and core trigger HMR
...
The core `spiffworkflow-frontend` directory is never modified. All extension logic is contained within `m8flow-frontend`.
...
All dependencies are installed in `m8flow-frontend/node_modules`, ensuring:
...
│  │  files) and checks for overrides in m8flow-frontend/src   │  │
│  └──────────────────────────────────────────────────────┘  │
│                          │                                   │
│        ┌─────────────────┼─────────────────┐                │
│        ▼                 ▼                 ▼                │
...
│  Files       │  │   Files      │  │   API        │
│              │  │              │  │              │
│ m8flow-      │  │ spiffworkflow│  │ localhost:   │
...
3. Calculates path: "components/SpiffLogo"
                    │
                    ▼
4. Checks: m8flow-frontend/src/components/SpiffLogo.tsx
                    │
                    ▼
5. Override exists? YES → Use m8flow-frontend/src/components/SpiffLogo.tsx
...
- If `m8flow-frontend/src/components/SpiffLogo.tsx` exists, it's used instead
...
└─────────────────────────────────────┘
      │                    │
      │ Yes                │ No (in m8flow-frontend)
      ▼                    ▼
┌──────────────────┐  ┌──────────────────┐
│ Relative import? │  │ Handle normally  │
│ (./Component)    │  │ (m8flow-frontend │
└──────────────────┘  │ logic)           │
      │               └──────────────────┘
      │ Yes
      ▼
...
1. **Relative Imports from Core Files** (`./SpiffLogo` in `SideNav.tsx`)
   ...
   - Checks if override exists at same path in `m8flow-frontend/src/`
...
3. Checks if `m8flow-frontend/src/components/SpiffLogo.tsx` exists
4. If yes, returns the override path

#### 2. Bare Module Resolution (from Core Files)

When a file in `spiffworkflow-frontend` imports a bare module (e.g., `import { jwtDecode } from 'jwt-decode'`), Vite needs to resolve it from `m8flow-frontend/node_modules`:
...
For relative imports within m8flow-frontend:
...
When you create `m8flow-frontend/src/components/SpiffLogo.tsx`, it's automatically used **everywhere** in the application - even when core files import it:
...
ContainerForExtensions.tsx → SideNav.tsx → SpiffLogo.tsx
                              ...
                              Override resolver intercepts!
                                              ↓
                              m8flow-frontend/src/components/SpiffLogo.tsx
```
...
**Override**: `m8flow-frontend/src/components/SpiffLogo.tsx`
```typescript
export default function SpiffLogo() {
...
Create a new view component in `m8flow-frontend/src/views/`:

```typescript
// m8flow-frontend/src/views/ReportsPage.tsx
...
```typescript
// m8flow-frontend/src/ContainerForExtensions.tsx
...
**1. Create the page** (`m8flow-frontend/src/views/ReportsPage.tsx`):
...
**2. Update App.tsx** (`m8flow-frontend/src/App.tsx`):
...
if (importer && importer.includes('/spiffworkflow-frontend/') 
  ...
  // Resolve it from m8flow-frontend/node_modules
  return this.resolve(source, path.resolve(extensionsDir, 'index.tsx'), { 
    skipSelf: true 
  });
}
...
2. **Context Switch**: Uses `this.resolve()` with a fake importer from `m8flow-frontend/src/index.tsx`
3. **Resolution**: Vite resolves the module from `m8flow-frontend/node_modules`

### Dependency Management

All dependencies must be installed in `m8flow-frontend`:
...
- Core code that is imported by m8flow-frontend
...
3. **Resolution**: SASS finds `@carbon/react` in `m8flow-frontend/node_modules/@carbon/react`
...
**Resolution**: SASS looks in `m8flow-frontend/node_modules/@carbon/react` (via `loadPaths`)
...
2. Create the same path under `m8flow-frontend/src/components/SpiffLogo.tsx`
...
**File**: `m8flow-frontend/vite.config.ts`
...
**File**: `m8flow-frontend/tsconfig.json`
...
**File**: `m8flow-frontend/src/index.tsx`
...
**File**: `m8flow-frontend/src/App.tsx`
...
✅ **Hot Reload**: Changes in both m8flow-frontend and core trigger HMR  
...
1. Create `m8flow-frontend/src/components/SpiffLogo.tsx`
